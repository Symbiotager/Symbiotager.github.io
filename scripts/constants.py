"""
Shared constants and utility functions for MonPotager data processing.
Extracted from the former models.py (database-dependent) into a standalone module.
"""
...
def most_complete(s1: str, s2: str) -> str:
    if s1 == s2:
        return s1
    if len(s1) > len(s2):
        return s1
    return clean_string(s2)


# Color palette for each category index
color = {
    0: "#0F0F0F", 1: "#1776b6", 2: "#ff7f0e", 3: "#9564bf", 4: "#f7b6d2",
    5: "#d62728", 6: "#24a221", 7: "#ffe778", 8: "#8d5649"
}

# Category names indexed by ID
categories = {
    1: "Légume", 2: "Fruit", 3: "Arômate", 4: "Fleur",
    5: "Nuisible", 6: "Auxiliaire", 7: "Céréale", 8: "Arbres"
}
...
# Reverse lookup: category name → category index
...
def print_fail_assoc(source, inter, target, line):
    s = f"Association '{source}' '{inter}' '{target}' n'est pas prise en compte (ligne {line})."
    print(FAIL + s + ENDC)
